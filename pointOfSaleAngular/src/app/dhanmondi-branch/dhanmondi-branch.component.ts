...
@Component({
  selector: 'app-dhanmondi-branch',
  templateUrl: './dhanmondi-branch.component.html',
  styleUrl: './dhanmondi-branch.component.css'
})
export class DhanmondiBranchComponent {
    ...
    onCategoryChange(index: number) {
      ...
      this.productService.findProductByCategoryName(selectedCategoryName).subscribe({
        next: (products: ProductModule[]) => {
          ...
          const bananiProducts = products.filter(product => product.branch.branchName === 'Dhanmondi');
    ...
    createSales() {
      ...
      this.salesService.createSales(this.sale).subscribe({
        next: res => {
          ...
          this.router.navigate(['dhanmondiinvoice'], { queryParams: { sale: JSON.stringify(this.sale) } });
        },
        error: error => { console.log(error); }
      });
    }
